...
-- Deduplicate existing rows before creating unique index (idempotent — safe to re-run)
...
-- Unique index so ON CONFLICT (time, user_id, metric, source) DO NOTHING works
